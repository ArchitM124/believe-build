...
- PASS COMPLETION IS A SEPARATE CLAIM: seeing the ball leave a player's hands toward a teammate proves only that a pass was THROWN. "Caught", "received", "completed" require seeing the receiver ESTABLISH CONTROL — ball secured in their hands followed by a hold, dribble, or deliberate next action. Track the ball's FLIGHT to its endpoint: if it sails past, lands behind, or bounces away untouched, say exactly that ("the pass sailed behind the teammate in black near half court — nobody in black touched it"). A player who is not looking at the ball — head turned, running downcourt facing away — is NOT catching it; when visible at the ball's arrival, record where the intended receiver was looking. End every observed pass with exactly one of: caught-and-controlled / touched-but-lost / never-touched / arrival-not-visible. Never default to "completed".
...
- PASS GRADING: a pass was "completed" or "successful" ONLY if the log shows the receiver establishing control (caught-and-controlled). If the log says the ball was never touched, arrived behind the receiver, or its arrival was not visible, the connection FAILED or is unknown — never credit a successful pass, and never blame the receiver for a ball the log does not show reaching them. On a failed connection, assign fault where the log points: a ball thrown behind or ahead of a moving teammate, or at a teammate who was not looking, is on the PASSER — an unready receiver is a reason to hold the ball, not an excuse for the throw.
